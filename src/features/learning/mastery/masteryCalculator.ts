...
export function getWeakestSkill(skills: SkillScores): ExerciseType {
  return (Object.entries(skills) as [ExerciseType, number][]).sort(
    (a, b) => a[1] - b[1],
  )[0][0];
}
export function updateSkillScore(
  ...
  const speedPenalty =
    responseTimeMs > 10_000 ? 0.1 : responseTimeMs > 6_000 ? 0.05 : 0;
  return clamp(
    current * 0.7 + Math.max(0, (correct ? 1 : 0) - speedPenalty) * 0.3,
  );
}
