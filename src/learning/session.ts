...
import type { LearningProgress, LearningSession, StudentWord, Word } from "../domain/types";
import type {
  WordLearningStats,
} from "../features/learning/types";
import { toLearningWord } from "../hooks/useLearningSession";

export type LearningWordType = "due" | "new" | "mixed";
...
export async function buildSession(
  ...
  let cards = await db.studentWords
    .where("studentId")
    .equals(studentId)
    .toArray();
  ...
  if (topicId) {
    const wordIds = new Set(
      allWords.filter((w) => belongsToTopic(w, topicId!)).map((w) => w.id),
    );
  ...
  if (
    canOpenNewWords &&
    (mode === "new" || (mode === "mixed" && dueCards.length <= maxBacklog))
  ) {
    const result = await addNewWords(
      studentId,
      topicId,
      newWordsLimit,
    );
  ...
  let reviewCards =
    mode === "new" ? [] : prioritizeCards(dueCards, allWords, maxBacklog);
    ...
    card.learningProgress
      ...
      : {
          ...
          learningProgress: {
            ...
            mastery: 0,
            weakestSkill: "recognition" as const,
            state: "new" as const,
            attempts: 0,
            correctAnswers: 0,
            wrongAnswers: 0,
...
function prioritizeCards(
  ...
  return buildReviewQueue(words.map(toLearningWord), statsMap, limit).map(
    (item) => cardByWordId.get(item.word.id)!,
  );
}
...
export function toLearningStats(card: StudentWord, word: Word): WordLearningStats {
